...
public class XmlElement {
    ...
    public void addChild(XmlElement child) {
        String childName = child.getName();//activity-state
    ...
    public XmlElement getChildElement(String childName) {
        ...
        if (namedChildren != null) {
            if (namedChildren.size() == 1) {
                ...
                throw new RuntimeException("expected only one child-element '" + childName + "' of element '" + name
                        + "' while there were " + namedChildren.size());
            }
        }
        return child;
    }
