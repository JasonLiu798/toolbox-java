...
public class JSONJacksonUtil {
    ...
    private static Logger logger = LoggerFactory.getLogger(JSONJacksonUtil.class);
    private ObjectMapper mapper;

    public JSONJacksonUtil() {

    }

    public JSONJacksonUtil(Inclusion inclusion) {
    ...
    public JSONJacksonUtil(Inclusion inclusion, boolean isIgnore) {
        mapper = new ObjectMapper();
        // 设置输出包含的属性
        mapper.getSerializationConfig().withSerializationInclusion(inclusion);
        // 设置输入时忽略JSON字符串中存在而Java对象实际没有的属性
     ...
     */
    public static JSONJacksonUtil buildNormalBinder() {
        return new JSONJacksonUtil(Inclusion.ALWAYS);
    }
     ...
     * @return JSONHelper
     */
    public static JSONJacksonUtil buildNormalIgnoreBinder() {
        return new JSONJacksonUtil(Inclusion.ALWAYS, true);
    }

    /**
     * 创建只输出非空属性到Json字符串的Binder.
     */
    public static JSONJacksonUtil buildNonNullBinder() {
        return new JSONJacksonUtil(Inclusion.NON_NULL);
    }

    /**
     * 创建只输出初始值被改变的属性到Json字符串的Binder.
     */
    public static JSONJacksonUtil buildNonDefaultBinder() {
        return new JSONJacksonUtil(Inclusion.NON_DEFAULT);
    }
    ...
    public <T> T parse(String jsonString, Class<T> clazz) {
        if (StringCheckUtil.isEmpty(jsonString)) {
            return null;
        }
        try {
            return mapper.readValue(jsonString, clazz);
        } catch (IOException e) {
            logger.warn("parseInner json string error:" + jsonString, e);
            return null;
        }
    }
    ...
    public void setDateFormat(String pattern) {
        if (StringCheckUtil.isEmpty(pattern)) {
