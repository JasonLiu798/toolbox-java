 ...
 * @author jasonliu
 */
public class DateAfter extends ValidatorBase {

    private static final String DFT_MSG = "日期需要在 %s,%s 之间";

    private Long min;//长度最小参考值
    private Long max;//长度最大参考值

    public DateAfter(Long min, Long max, String msg) {
        init(min, max, msg, false);
    }

    public DateAfter(Long min, Long max) {
        init(min, max, DFT_MSG, true);
    }

    public DateAfter(Long max) {
