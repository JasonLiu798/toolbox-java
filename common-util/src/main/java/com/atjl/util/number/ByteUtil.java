...
public class ByteUtil {
    ...
    public static final String bytes2BcdStrAccordNumber(byte[] bs, int offset, int end, boolean littleEndian) {
        ...
        if (!littleEndian) {
            for (int i = offset; i <= end; i++) {
                byte e = bs[i];
                str.append(Integer.toHexString(e >> 4 & 0xF));
                str.append(Integer.toHexString(e & 0xF));
            }
        } else {
    ...
    public static double convertDecimal(double t, int scale) {
        StringBuilder str = new StringBuilder(Double.toString(t));
        while (str.length() < 8) {
            str.append( "0");
        }
        String sstr = str.toString().substring(0, scale);
        double tmp = Double.parseDouble(sstr);
        return tmp;
    }
